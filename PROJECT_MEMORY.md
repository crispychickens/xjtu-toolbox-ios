...
| Overall first iOS release readiness | 83% | Feature-complete enough for a controlled alpha, with Release dependency wiring, simulator runtime smoke, KMP bridge and preview UI-smoke coverage, privacy manifest, explicit versioning, compiled AppIcon assets, a documented crash/log policy, and unsigned device-archive validation now productionized. It is still not ready for unattended production release without live validation, remote CI, signed archive/export, App Store privacy-disclosure review, runtime device-log inspection, and TestFlight evidence. |
...
- A current unsigned generic-device Release archive for `1.0.0 (1)` passes identity, iPhone-only arm64, compiled AppIcon asset, privacy/ATS, dSYM, Debug-auth-marker, and known unreviewed crash-SDK-marker validation. Remote macOS CI, final signing/export, App Store privacy review, runtime device-log inspection, App Store listing visuals, and TestFlight evidence remain pending.
