...
- CodeGraph is local and ignored by Git. On 2026-06-14 it was current at 267 files, 8,806 nodes, and 18,671 edges.
...
| Shared architecture and fixture coverage | 91% | Core boundaries and fail-closed parsers are established; the iOS cache/store, launch/dependency-policy, auth-state, and preview UI-smoke XCTest baseline now exists. School endpoint churn, action workflows, and broader iOS UI coverage remain residual risk. |
...
| iOS product/release readiness | 65% | Debug and Release simulator builds pass, the XCTest suite covers cache, launch/dependency policy, auth-state invariants, and a preview UI smoke path, simulator Release smoke proves preview args do not enter the shell, and a validated unsigned generic-device archive now proves version, device-family, privacy/ATS, arm64, and dSYM integrity. CI includes the same archive gate. Remote CI, final app identity/signing/export, TestFlight, App Store privacy-disclosure review, crash/log policy, and assets remain incomplete. |
...
- Test shape: shared has substantial fixture-driven common/JVM tests; Android tests are sparse; iOS has initial cache/store, launch/dependency-policy, auth-state, and preview UI-smoke XCTest coverage but limited KMP bridge coverage.
...
| R1 | Expand iOS automated coverage beyond cache, launch/dependency policy, auth-state, and preview UI-smoke tests: add targeted KMP bridge tests and obtain a passing remote macOS CI result. | H | high |
