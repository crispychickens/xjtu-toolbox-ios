...
| iOS product/release readiness | 58% | Debug and Release simulator builds pass, the XCTest suite covers cache, launch/dependency policy, auth-state invariants, and a preview UI smoke path, simulator Release smoke proves preview args do not enter the shell, and macOS CI includes shared, iOS test, and Release build steps. Remote CI, signing/archive/TestFlight/privacy/assets work is not established. |
...
| Overall first iOS release readiness | 78% | Feature-complete enough for a controlled alpha, with Release dependency wiring, simulator runtime smoke, and preview UI-smoke coverage now productionized. It is still not ready for unattended production release without live validation, remote CI, signing, privacy, archive, and TestFlight evidence. |
