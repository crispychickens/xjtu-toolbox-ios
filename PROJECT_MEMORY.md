...
| iOS product/release readiness | 70% | Debug and Release simulator builds pass, the XCTest suite covers cache, launch/dependency policy, auth-state invariants, sanitized Debug diagnostic metadata, KMP bridge contracts, and a preview UI smoke path. Simulator Release smoke proves preview args do not enter the shell, while the validated unsigned generic-device archive proves version, device-family, privacy/ATS, arm64, dSYM integrity, and absence of the Debug-auth marker and known unreviewed crash-SDK markers. Remote CI, final app identity/signing/export, TestFlight, App Store privacy-disclosure review, runtime device-log inspection, and assets remain incomplete. |
...
| Overall first iOS release readiness | 82% | Feature-complete enough for a controlled alpha, with Release dependency wiring, simulator runtime smoke, KMP bridge and preview UI-smoke coverage, privacy manifest, explicit versioning, a documented crash/log policy, and unsigned device-archive validation now productionized. It is still not ready for unattended production release without live validation, remote CI, signed archive/export, App Store privacy-disclosure review, runtime device-log inspection, and TestFlight evidence. |
...
- Shared has substantial fixture-driven common/JVM tests and current `./gradlew :shared:check` passes. Android tests remain sparse; endpoint churn, action workflows, and real adapter failure/cancellation behavior remain residual gaps.
- Full generated-project `xcodebuild test` passes 22/22 locally: 20 unit/contract tests plus 2 preview UI smoke tests. Coverage includes cache/store, launch/dependency policy, auth-state invariants, sanitized Debug diagnostic metadata, KMP auth/feature bridge contracts, default Debug login launch, and preview schedule rendering. Broader UI workflows remain uncovered.
...
- A current unsigned generic-device Release archive for `1.0.0 (1)` passes identity, iPhone-only arm64, privacy/ATS, dSYM, Debug-auth-marker, and known unreviewed crash-SDK-marker validation. Remote macOS CI, final signing/export, App Store privacy review, runtime device-log inspection, and TestFlight evidence remain pending.
...
| R1 | Obtain a passing remote macOS CI result for the current shared, 20-unit/contract, 2-UI-smoke, and unsigned-archive gates; add further KMP platform-adapter/error-path tests only when evidence exposes a gap. | M | high |
...
| R1 | Finalize App Store icon and release-facing visual assets, then verify them in the archived app and App Store Connect. | M | medium |
| R1 | Complete App Store privacy questionnaire/disclosure review against the manifest, runtime behavior, and first-release no-telemetry policy. | H | high |
...
3. Complete app identity/signing/export, release assets, App Store privacy disclosures, and the TestFlight pipeline; ship a controlled alpha.
