...
- A current unsigned generic-device Release archive for `1.0.0 (1)` passes identity, iPhone-only arm64, compiled AppIcon asset, privacy/ATS, dSYM, Debug-auth-marker, and known unreviewed crash-SDK-marker validation. Local App Store privacy-review evidence and a privacy-policy draft are documented. The iOS validation workflow now mirrors the shared/XCTest/unsigned-archive gates with manual trigger, version logging, deterministic simulator boot, and XCTest failure artifacts. A passing remote macOS CI run, final signing/export, App Store Connect privacy entry/public URLs, runtime device-log inspection, App Store listing visuals, and TestFlight evidence remain pending.
