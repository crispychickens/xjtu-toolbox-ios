...
- CodeGraph is local and ignored by Git. On 2026-06-14 it was current at 267 files, 8,805 nodes, and 18,695 edges.
...
| iOS product/release readiness | 60% | Debug and Release simulator builds pass, the XCTest suite covers cache, launch/dependency policy, auth-state invariants, and a preview UI smoke path, simulator Release smoke proves preview args do not enter the shell, the app bundles a privacy manifest, and macOS CI includes shared, iOS test, and Release build steps. Remote CI, signing/archive/TestFlight, App Store privacy-disclosure review, and assets work are not established. |
...
| Overall first iOS release readiness | 79% | Feature-complete enough for a controlled alpha, with Release dependency wiring, simulator runtime smoke, preview UI-smoke coverage, and the privacy manifest now productionized. It is still not ready for unattended production release without live validation, remote CI, signing/archive, App Store privacy-disclosure review, and TestFlight evidence. |
...
| R1 | Complete remaining iOS release engineering: app identity/signing, icons/assets, App Store privacy-disclosure review, archive/export validation, versioning, crash/log policy, TestFlight pipeline, and release checklist. | VH | xhigh |
