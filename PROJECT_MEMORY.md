...
- CodeGraph is local and ignored by Git. On 2026-06-14 it was current at 268 files, 8,813 nodes, and 18,694 edges.
...
| Shared architecture and fixture coverage | 93% | Core boundaries and fail-closed parsers are established; iOS now tests cache/store, launch/dependency policy, auth-state, preview UI smoke, and the KMP auth/first-release-feature bridge contract against the generated framework. School endpoint churn, action workflows, platform-adapter failure paths, and broader iOS UI coverage remain residual risk. |
...
| iOS product/release readiness | 67% | Debug and Release simulator builds pass, the XCTest suite covers cache, launch/dependency policy, auth-state invariants, KMP bridge contracts, and a preview UI smoke path, simulator Release smoke proves preview args do not enter the shell, and a validated unsigned generic-device archive proves version, device-family, privacy/ATS, arm64, and dSYM integrity. CI includes the same test/archive gates. Remote CI, final app identity/signing/export, TestFlight, App Store privacy-disclosure review, crash/log policy, and assets remain incomplete. |
...
| Overall first iOS release readiness | 81% | Feature-complete enough for a controlled alpha, with Release dependency wiring, simulator runtime smoke, KMP bridge and preview UI-smoke coverage, privacy manifest, explicit versioning, and unsigned device-archive validation now productionized. It is still not ready for unattended production release without live validation, remote CI, signed archive/export, App Store privacy-disclosure review, and TestFlight evidence. |
...
- 2026-06-14: iOS auth-state XCTest coverage was added. The generated test suite passed 16/16 locally, and the Release simulator build still passed after regenerating the Xcode project. Coverage now includes AuthStore login session-context persistence, logout cleanup, site-verification-as-inline-reauth after an authenticated shell, site-verification behavior without shell context, account-choice undergraduate defaulting, preview auto-login no-op without launch args, cache/store behavior, and launch/dependency policy.
- 2026-06-14: A generated iOS UI-test target and preview smoke suite were added. Full `xcodebuild test` passed 18 tests locally: 16 unit tests plus 2 UI smoke tests proving default Debug launch stays on the login screen and Debug preview auto-login reaches the `日程` tab with preview courses (`高等数学`, `大学物理`, `线性代数`). Release simulator build still passed after adding the UI-test target.
- 2026-06-14: iOS privacy manifest was added and bundled into the Release simulator app. `PrivacyInfo.xcprivacy` declares no tracking, no tracking domains, no collected data types, and the UserDefaults required-reason API category with reason `CA92.1`. `plutil -lint` passed, the Release simulator build copied the manifest into `XJTUToolboxIOS.app`, and full `xcodebuild test` still passed 18/18 locally.
- 2026-06-14: iOS release versioning and unsigned device-archive validation were established. `iosApp/project.yml` now owns `1.0.0 (1)` and the first-release iPhone-only target; `iosApp/scripts/validate-release-archive.sh` validates identity/version, arm64, privacy/ATS, and matching dSYM UUIDs, with an optional signed-archive gate. A current generic-device Release archive passed the validator, full `xcodebuild test` passed 18/18, and `./gradlew :shared:check` passed. CI now builds and validates the unsigned device archive instead of only building Release for simulator.
- 2026-06-14: The archive checkpoint exposed a pre-existing AuthStore state leak: explicit login could retain a stale authenticated-session context when entering account choice. A deterministic regression test now seeds that prior context; explicit login clears it before challenges, while the separate site-verification path preserves inline-reauthorization semantics.
- 2026-06-14: Targeted iOS KMP bridge contract tests were added against `XjtuToolboxPreviewFactory` and the generated framework rather than Swift spies. They cover auth challenge/failure/access-mode mapping and every first-release `CoreFeatureService` method through `KmpFeatureProviderAdapter`. Full `xcodebuild test` passed 20/20 locally: 18 unit/contract tests plus 2 UI smoke tests; `./gradlew :shared:check` also passed.
- Test shape: shared has substantial fixture-driven common/JVM tests; Android tests are sparse; iOS has cache/store, launch/dependency-policy, auth-state, KMP preview-contract, and preview UI-smoke XCTest coverage. Real adapter failure/cancellation behavior and broader UI flows remain residual gaps.
...
| R1 | Obtain a passing remote macOS CI result for the current shared, 18-unit/contract, 2-UI-smoke, and unsigned-archive gates; add further KMP platform-adapter/error-path tests only when evidence exposes a gap. | M | high |
...
| R1 | Complete remaining iOS release engineering: final app identity/signing, icons/assets, App Store privacy-disclosure review, signed archive/export validation, crash/log policy, and TestFlight pipeline. | VH | xhigh |
...
3. Complete signing, App Store privacy disclosures, archive, and TestFlight work; ship a controlled alpha.
