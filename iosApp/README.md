...
- Real first-release feature diagnostics compile only in Debug and require `-XJTURealFeatureValidation` together with `-XJTURealFirstReleaseCore`. They wait for owner-entered or restored authentication before starting, record only structural status/counts under the `debugRealFeatureValidationResults` UserDefaults key, resume from the interrupted feature after site verification, and never write course names, coupon names, seat ids, credentials, tokens, or response bodies.
- Inline site verification remains user-driven by default. Controlled Debug validation can opt into `-XJTUAutoSiteVerification`; those attempts require an authenticated shell context, go through shared `AuthManager.beginSiteVerification`, never submit captcha/MFA values, and are limited to at least 1 second between attempts and one attempt per site per 5-minute window.
...
`project.yml` is also the source of truth for the iOS marketing version, build number, and first-release iPhone-only device family. `scripts/validate-release-archive.sh` inspects a generic-device archive for identity/version consistency, arm64 and dSYM integrity, privacy/ATS policy, and optional signing evidence. `scripts/run-signed-release-gate.sh` injects a release-owner team id only at command time, builds a signed Release archive, runs strict signed validation, and optionally exports an IPA from an approved `ExportOptions.plist`. Use `docs/ios-release-checklist.md` for the complete promotion gate.
...
6. Use `iosApp/scripts/run-real-feature-validation.sh` for a controlled `-XJTURealFirstReleaseCore -XJTURealFeatureValidation -XJTUAutoSiteVerification` diagnostic run. If the app starts unauthenticated, enter credentials manually; the runner starts after authentication succeeds. It sequentially probes library-seat read state, coupon load/filter states, and school-wide course first/next-page plus representative filter searches, pauses on site verification, resumes from the interrupted feature after automatic/manual site verification completes, and stores sanitized result lines in `debugRealFeatureValidationResults`. It does not perform library booking/swap or coupon actions. While this diagnostic flag is active, regular SwiftUI auto-loads are disabled so the runner owns the real request sequence. Set `IOS_REAL_FEATURE_RESULTS_PATH=/absolute/path/results.txt` to save only those sanitized lines as local evidence; set `IOS_REAL_FEATURE_ACCESS_MODE=automatic`, `normal`, or `webvpn` to force the Debug launch access mode for route validation; set `IOS_REAL_FEATURE_SKIP_BUILD=1 IOS_REAL_FEATURE_SKIP_INSTALL=1` only when reusing an already installed diagnostic build.
7. Do not ask testers to prefer the browser-login button for current validation: live testing on 2026-05-30 showed school CAS rejects the unregistered `xjtutoolbox://auth` service. Browser login becomes viable only after CAS accepts an app callback/Universal Link or another app-readable artifact.
