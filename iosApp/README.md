...
6. Use `iosApp/scripts/run-real-feature-validation.sh` for a controlled `-XJTURealFirstReleaseCore -XJTURealFeatureValidation -XJTUAutoSiteVerification` diagnostic run. If the app starts unauthenticated, enter credentials manually; the runner starts after authentication succeeds. It sequentially probes library seats, coupons, and school-wide course search, pauses on site verification, resumes from the interrupted feature after automatic/manual site verification completes, and stores sanitized result lines in `debugRealFeatureValidationResults`. Set `IOS_REAL_FEATURE_RESULTS_PATH=/absolute/path/results.txt` to save only those sanitized lines as local evidence; set `IOS_REAL_FEATURE_SKIP_BUILD=1 IOS_REAL_FEATURE_SKIP_INSTALL=1` only when reusing an already installed diagnostic build.
