...
- `.github/workflows/ios-release-gate.yml` runs on `main` pushes and manual `workflow_dispatch`; it invokes `iosApp/scripts/run-automated-release-gate.sh`, including all 38 XCTest cases (31 unit/contract plus 7 UI smoke), the unsigned generic-device Release archive, and archive validator. The temporary `ios-release-validation` branch runs the same workflow in archive-only diagnostic mode so validator/Xcode-runner failures can be fixed quickly; `ios-full-release-validation` runs the full workflow from a push so full remote gates do not depend on browser workflow dispatch. Use a `main` or manual full workflow run for release-owner evidence before signing/TestFlight work.
